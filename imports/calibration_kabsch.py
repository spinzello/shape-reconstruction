...
import imports.calculate_rmsd_kabsch as rmsd
import numpy as np
from imports.helper_functions import cv_find_chessboard, get_chessboard_points_3D, get_depth_at_pixel, convert_depth_pixel_to_metric_coordinate
...
class PoseEstimation:
	...
	def __init__(self, depth_frames, infrared_frames, serials, intrinsic, chessboard_params):
		assert(len(chessboard_params) == 3)
		self.depth_frames = depth_frames
		self.infrared_frames = infrared_frames
	...
	def get_chessboard_corners_in3d(self):
		...
		"""
		corners3D = {}
		for serial, depth_frame, infrared_frame in zip(self.serials, self.depth_frames, self.infrared_frames):
