...
def post_process_depth_frame(frameset, 
    ...
    spatial_magnitude    : double
                           Iterations of the filter. We recommend using 2 (default).
    spatial_smooth_alpha : double
                           The alpha value for spatial filter based smoothening (default = 0.5). Alpha=1 - no filter . Alpha = 0 - infinite filter.
    spatial_smooth_delta : double
                           The delta value for spatial filter based smoothening
                           Default is 20, but try 8 as well.
    temporal_smooth_alpha: double
                           The alpha value for temporal filter based smoothening (default = 0.4). Alpha=1 -> no filter . Alpha = 0 -> infinite filter.
    temporal_smooth_delta: double
                           The delta value for temporal filter based smoothening (default = 20). Step-size boundary. Establishes the threshold used to preserve surfaces (edges)
    ...
    temporal_filter = rs.temporal_filter(smooth_alpha=temporal_smooth_alpha, smooth_delta=temporal_smooth_delta, persistence_control=persistence)
    ...
    # temporal_filter.set_option(rs.option.holes_fill, 2)

    hole_filling_filter.set_option(rs.option.holes_fill, 2)

    # Apply the filters
    if frameset.is_depth_frame():
        filtered_frame = decimation_filter.process(frameset)
        filtered_frame = spatial_filter.process(filtered_frame)
        filtered_frame = temporal_filter.process(filtered_frame)
        # filtered_frame = hole_filling_filter.process(frameset)

    elif frameset.is_frameset():
        filtered_frame = decimation_filter.process(frameset).as_frameset()
        filtered_frame = spatial_filter.process(filtered_frame).as_frameset()
        filtered_frame = temporal_filter.process(filtered_frame).as_frameset()
        # filtered_frame = hole_filling_filter.process(frameset).as_frameset()

    else:
        Warning('Wrong input to post_process_depth_frame()')
...
class DeviceManager:
    ...
    def enable_emitter(self, on=True, always=False, max_power=False):
        """
        ...
        for (device_serial, device) in self._enabled_devices.items():
            ...
            sensor.set_option(rs.option.emitter_always_on, 1 if always else 0)
            if max_power:
        ...
        while len(frames) < len(self._enabled_devices.items()) :
            for (serial, device) in self._enabled_devices.items():
                ...
                while True:
                    ...
                    if frameset: break
                # frameset = post_process_depth_frame(frameset)
    ...
    def poll_frameset(self):
        frameset = []
        while len(frameset) < len(self._enabled_devices.items()) :
            for (serial, device) in self._enabled_devices.items():
                while True:
                    frame = device.pipeline.poll_for_frames()
                    if frame: break
                frameset.append(frame) #frameset will be a pyrealsense2.composite_frame object

        return frameset

    def poll_aligned_frameset(self, align):
        frameset = []
        while len(frameset) < len(self._enabled_devices.items()) :
            for (serial, device) in self._enabled_devices.items():
                frameset.append(align.process(device.pipeline.poll_for_frames())) #frameset will be a pyrealsense2.composite_frame object
